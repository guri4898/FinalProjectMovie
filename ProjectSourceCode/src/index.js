...
app.get("/movie/:title", (req, res) => {
  ...
  db.one(query, [title])
    ...
    .catch(function (error) {
      res.status(500).render("pages/singleMovie", {
        error: true,
        display: true,
        exists: false,
      });
...
app.post("/addMovie", async (req, res) => {
  ...
  const movieToGenresQuery =
    "INSERT INTO movie_genres (movie_id, genre_id) VALUES ($1, $2);";

  const movieId = await db.one(query, [
    title,
    year,
    defaultRating,
    director,
...
app.get("/home", async (req, res) => {
  try {
    //gets the url for the image from the database
    const imageUrls = await fetchImageUrlsFromDatabase();

    //render the carousel image from the urls in the database
    res.render("carousel", { images: imageUrls });
  } catch (error) {
